...
func TestFilterSchedulerCredentialsKeepsCodexTicketIdentity(t *testing.T) {
	filtered := filterSchedulerCredentials(map[string]any{
		"plan_type":          "plus",
	...
	require.Equal(t, "plus", filtered["plan_type"])
	require.Equal(t, "acc-1", filtered["chatgpt_account_id"])
	require.Equal(t, "user@example.com", filtered["email"])
	require.NotContains(t, filtered, "access_token")
	require.NotContains(t, filtered, "refresh_token")
}
...
func TestSchedulerMetadataAccountKeepsOpenAISubscriptionIdentity(t *testing.T) {
	...
	require.Equal(t, "acc-1", metadata.GetCredential("chatgpt_account_id"))
	require.Equal(t, "user@example.com", metadata.GetCredential("email"))
