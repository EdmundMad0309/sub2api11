...
func currentCodexHarvestNode(ctx context.Context) string {
	if ctx == nil {
		ctx = context.Background()
	}
	if now := observeCodexHarvestSidecar(ctx).Now; now != "" {
		return now
	}
...
func loadCodexHarvestSidecarController() cachedSidecarController {
	...
	path := filepath.Join(dataDir, "mihomo-codex", "config.yaml")
