...
func ticketIdentityMaterial(account *Account) (chatgptAccountID, email string) {
	if account == nil {
		return "", ""
	}
	return strings.TrimSpace(account.GetCredential("chatgpt_account_id")), strings.TrimSpace(account.GetCredential("email"))
}

func ticketIdentity(account *Account) string {
	chatgptAccountID, email := ticketIdentityMaterial(account)
	if chatgptAccountID == "" && email == "" {
		return ""
	}
	return fmt.Sprintf("%x", sha256.Sum256([]byte(chatgptAccountID+"\x00"+email)))
}

// ticketIdentityMatches rejects a ticket only when the account snapshot has
// workspace identity material AND it disagrees with the ticket. Scheduler
// metadata strips tokens and used to strip chatgpt_account_id/email too; hashing
// those empty fields produced a stable "blank" identity that never matched a
// harvested ticket, so fail-closed admission skipped every OpenAI account.
...
func (s *OpenAIGatewayService) openAICodexTicketHarvestExcluded(account *Account) bool {
	...
	if err != nil || scope.Mode != "selected" {
		return false
	}
