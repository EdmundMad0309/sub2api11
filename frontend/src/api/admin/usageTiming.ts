...
export interface TimingAttempt {
  number: number; account_id: number; proxy_id: number; start_ms: number; end_ms: number | null
  status: number; error?: string; reused: boolean | null; request_bytes: number; response_bytes: number
  body_eof: boolean; events: Record<string, number>
}
export interface RequestTiming {
  version: number; trace_id: string; started_at: string; total_ms: number; status: number; canceled: boolean
