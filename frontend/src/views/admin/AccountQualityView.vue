    ...
    <div class="space-y-6">
      ...
      <div class="card overflow-x-auto">
        <table class="w-full text-left text-sm">
          ...
          <tbody><tr v-for="plan in plans" :key="plan.id" class="border-t dark:border-dark-600">
            ...
            <td class="p-3"><span :class="plan.enabled ? 'text-green-600' : 'text-gray-500'">{{ t(plan.enabled ? 'qualityOps.enabled' : 'qualityOps.paused') }}</span><div class="text-xs text-gray-500">{{ plan.enabled ? date(plan.next_run_at) : '—' }}</div></td>
      ...
      <section v-if="historyPlan" class="card space-y-3 p-5">
        ...
        <div v-for="result in results" :key="result.id" class="rounded border p-3 dark:border-dark-600">
          <div class="flex flex-wrap justify-between gap-2 text-sm"><span>{{ date(result.started_at) }}</span><span :class="result.status === 'success' ? 'text-green-600' : 'text-red-600'">{{ t(result.status === 'success' ? 'qualityOps.passed' : result.error_message === 'answer_mismatch' ? 'qualityOps.wrongAnswer' : 'qualityOps.requestError') }}</span><span>{{ actionLabel(result.quality_action) }}</span></div>
...
import { listQualityPlans, runQualityPlan } from '@/api/admin/accountQuality'
...
function defaults() {
  return { model_id: '', cron_expression: '*/30 * * * *', enabled: true, max_results: 100, auto_recover: false,
    pelican_config: { question_kind: 'candy' as const, prompt: CANDY_PROMPT, reasoning_effort: 'high', parallel_count: 1,
      quality: { expected_answer: '21', action: 'remove_groups' as 'remove_groups' | 'disable_scheduling', remove_group_ids: [] as number[], auto_restore: false } } }
}
...
async function load() { await task(async () => { [plans.value, groups.value] = await Promise.all([listQualityPlans(), groupsAPI.getAllIncludingInactive()]); if (historyPlan.value) results.value = await scheduledTests.listResults(historyPlan.value.id, 100) }) }
function newPlan() { editing.value = null; form.value = defaults(); selectedAccounts.value = []; showForm.value = true; void searchAccounts() }
function edit(plan: ScheduledTestPlan) { editing.value = plan.id; form.value = { ...defaults(), model_id: plan.model_id, cron_expression: plan.cron_expression, enabled: plan.enabled, max_results: plan.max_results, pelican_config: { ...defaults().pelican_config, ...JSON.parse(JSON.stringify(plan.pelican_config)) } }; showForm.value = true }
